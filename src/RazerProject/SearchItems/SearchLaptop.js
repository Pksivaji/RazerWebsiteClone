...
import LaptopData from '../JsonDatas/laptopsData.json';
    ...
    <div className='laptopsDiv'>
      ...
      <div className='filterDiv'>
          ...
          {LaptopData.map((laptop) => (
            <div key={laptop.brand}>
              <label>
                <input
                  type='checkbox'
                  checked={selectedBrands.has(laptop.brand)}
                  onChange={() => brandHandler(laptop.brand)} // Use arrow function for consistent 'this'
                />
                {
                  laptop.brand
                }
        ...
        <div className='Lapfilter2'>
          ...
          {LaptopData.map((laptop) => (
            <div key={laptop.rating}>
              <label>
                <input
                  type='checkbox'
                  checked={selectedRating.has(laptop.rating)}
                  onChange={() => ratingHandler(laptop.rating)} // Use arrow function for consistent 'this'
                />
                {
                  laptop.rating
                }
        ...
        {LaptopData.filter((item) => (
          ...
          <div key={item.id} className='d-flex m-2 flex-md-column flex-sm-column flex-column flex-lg-row rounded p-2 itemDiv'>
            ...
            <div className='w-50 px-5 p-4 rounded div2'>
              ...
              <h6>{item.graphics} + {item.ram}(RAM)</h6>
              <h6>{item.os} ({item.generation} Gen)</h6>
              <h6>{item.memory} ({item.bits})</h6>
              <h6>{item.display_size* 2.50}cm Display</h6>
             </div>
            {/* -----------div 3------------- */}
            <div className='w-25 text-center align-center py-lg-5 div3'>
              <h3>₹{(Math.floor(item.price * 82)).toLocaleString()} Rs</h3>
              <h6 className='text-muted'>
                <del>{item.price*92}</del> 
